...
const ManageDrivers = () => {
  ...
  return (
    <main className="relative isolate min-h-screen px-6 py-10">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_10%_20%,rgba(31,110,255,0.35),transparent_55%),radial-gradient(circle_at_80%_0%,rgba(236,72,153,0.25),transparent_40%)]" />
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-slate-400">Admin Suite</p>
            <h1 className="text-4xl font-semibold tracking-tight text-white">Driver Roster</h1>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <GlassCard>
            <p className="text-xs uppercase tracking-[0.5em] text-slate-400">Total</p>
            <p className="mt-2 text-4xl font-semibold">{drivers.length}</p>
            <p className="text-sm text-slate-400">registered drivers</p>
          </GlassCard>
          <GlassCard>
            <p className="text-xs uppercase tracking-[0.5em] text-slate-400">Assigned</p>
            <p className="mt-2 text-4xl font-semibold">{assignedDrivers}</p>
            <p className="text-sm text-slate-400">linked to a bus</p>
          </GlassCard>
          <GlassCard>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {filteredDrivers.map((driver) => (
              <GlassCard key={driver._id} className="relative">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm uppercase tracking-[0.4em] text-slate-400">{driver.username}</p>
                    <h3 className="mt-2 text-2xl font-semibold text-white">{driver.name || 'Unnamed driver'}</h3>
                    <p className="text-sm text-slate-300">{driver.phone || 'No phone on file'}</p>
                  </div>
                  <span
                    className={`badge ${
                      driver.assignedBusId ? 'bg-success-subtle text-success-emphasis' : 'bg-warning-subtle text-warning-emphasis'
                    }`}
                  >

...
const ManageBuses = () => {
  ...
  return (
    <main className="relative isolate min-h-screen px-6 py-10">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_20%_10%,rgba(59,130,246,0.35),transparent_55%),radial-gradient(circle_at_80%_0%,rgba(236,72,153,0.25),transparent_40%)]" />
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-slate-400">Fleet Studio</p>
            <h1 className="text-4xl font-semibold tracking-tight text-white">Bus command board</h1>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <GlassCard>
            <p className="text-xs uppercase tracking-[0.5em] text-slate-400">Buses</p>
            <p className="mt-2 text-4xl font-semibold">{buses.length}</p>
            <p className="text-sm text-slate-400">{activeBuses} currently routed</p>
          </GlassCard>
          <GlassCard>
            <p className="text-xs uppercase tracking-[0.5em] text-slate-400">Capacity</p>
            <p className="mt-2 text-4xl font-semibold">{totalCapacity}</p>
            <p className="text-sm text-slate-400">total seats available</p>
          </GlassCard>
          <GlassCard>
        ...
        <GlassCard className="grid gap-4 text-white lg:grid-cols-3">
          <label className="text-sm">
            <span className="text-xs uppercase tracking-[0.4em] text-slate-300">Route filter</span>
            <select
              className="mt-2 w-full rounded-2xl border border-white/20 bg-white/5 px-4 py-2 text-white"
              value={routeFilter}
          ...
          <label className="text-sm">
            <span className="text-xs uppercase tracking-[0.4em] text-slate-300">Assignment</span>
            <select
              className="mt-2 w-full rounded-2xl border border-white/20 bg-white/5 px-4 py-2 text-white"
              value={statusFilter}
          ...
          <div className="space-y-2 text-sm">
            <p className="text-xs uppercase tracking-[0.4em] text-slate-300">Quick actions</p>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          {filteredBuses.map((bus) => (
            <GlassCard key={bus._id} className="space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.4em] text-slate-400">{bus.numberPlate}</p>
                  <h3 className="mt-2 text-3xl font-semibold text-white">{bus.name}</h3>
                  <p className="text-sm text-slate-300">Capacity {bus.capacity}</p>
                </div>
                <span
              ...
              <div className="grid gap-2 text-sm md:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                  ...
                  <p className="mt-1 text-base text-white">{bus.route?.name || 'Unassigned'}</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Driver</p>
                  <p className="mt-1 text-base text-white">{bus.driver?.name || bus.driver?.username || 'Unassigned'}</p>
                </div>
              </div>

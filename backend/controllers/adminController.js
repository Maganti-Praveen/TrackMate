...
const fixStudentData = async (req, res) => {
  try {
    ...
    for (const student of students) {
      // Hardcode dummy stop location (Eluru area - Near Reddy Womens)
      student.stopCoordinates = { lat: 16.70, lng: 81.05 };
      // Updated to match the active trip bus ID from logs
      student.assignedBusId = "693e3a4c50a0d509be16173a";

      // Initialize pushSubscription if missing
    ...
    res.json({ message: `Fixed ${count} Students`, studentsFixed: count });
  } catch (error) {
